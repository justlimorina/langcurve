...
class DocStoreAdapter {
    ...
    async saveWord(word, source, rawPayload) {
        if (dbMode === 'PRODUCTION') {
            try {
    ...
    writeToJsonCache(word, source, rawPayload) {
...
class DbAdapter {
    async getStats() {
        ...
        const examplesCount = vocabs.filter((v) => v.user_example && v.user_example.trim() !== '').length;
        return {
    ...
    async createTopic(name, description) {
        ...
        const result = await sqliteDb.run('INSERT INTO topics (name, description) VALUES (?, ?)', [name.trim(), description ? description.trim() : null]);
    ...
    async upsertVocabulary(topicId, word, userExample) {
        if (dbMode === 'PRODUCTION') {
            try {
        ...
        if (existing) {
            await sqliteDb.run('UPDATE vocabularies SET topic_id = ?, user_example = ? WHERE id = ?', [topicId, userExample ? userExample.trim() : null, existing.id]);
            return { id: existing.id, topicId, word: word.trim().toLowerCase(), userExample };
        }
        else {
            const res = await sqliteDb.run('INSERT INTO vocabularies (topic_id, word, user_example, easiness, interval, repetitions, due_date) VALUES (?, ?, ?, 2.5, 0, 0, ?)', [topicId, word.trim().toLowerCase(), userExample ? userExample.trim() : null, new Date().toISOString()]);
            return { id: res.lastID, topicId, word: word.trim().toLowerCase(), userExample };
        }
    }
    async updateExample(vocabId, userExample) {
        if (dbMode === 'PRODUCTION') {
            try {
    ...
    async recordSrsReview(word, quality) {
        ...
        if (row) {
            await sqliteDb.run('UPDATE vocabularies SET easiness = ?, repetitions = ?, interval = ?, due_date = ? WHERE id = ?', [nextEF, nextRepetitions, nextInterval, dueDate.toISOString(), row.id]);
        }
...
export const cacheAdapter = new RedisAdapter();
export const docStoreAdapter = new DocStoreAdapter();
...
export async function initializeSqliteFallback() {
  ...
  `);
    ...
    try {
        ...
        const hasDefinition = tableInfo.some((col) => col.name === 'definition');
        if (hasDefinition) {
    ...
    if (vocabCount?.count === 0) {
        await sqliteDb.run('INSERT INTO vocabularies (topic_id, word, user_example, easiness, interval, repetitions, due_date) VALUES (?, ?, ?, 2.5, 0, 0, ?)', [1, 'algorithm', 'The software developers designed a new algorithm to sort the user database more efficiently.', new Date().toISOString()]);
        await sqliteDb.run('INSERT INTO vocabularies (topic_id, word, user_example, easiness, interval, repetitions, due_date) VALUES (?, ?, ?, 2.5, 0, 0, ?)', [1, 'responsive', 'Our new web app is completely responsive, ensuring it looks beautiful on both mobile phones and wide desktop screens.', new Date().toISOString()]);
        await sqliteDb.run('INSERT INTO vocabularies (topic_id, word, user_example, easiness, interval, repetitions, due_date) VALUES (?, ?, ?, 2.5, 0, 0, ?)', [2, 'synergy', 'We hope the merger will create synergy between the two marketing departments.', new Date().toISOString()]);
        await sqliteDb.run('INSERT INTO vocabularies (topic_id, word, user_example, easiness, interval, repetitions, due_date) VALUES (?, ?, ?, 2.5, 0, 0, ?)', [3, 'itinerary', 'The travel agent provided us with a detailed itinerary for our 10-day trip to Japan.', new Date().toISOString()]);
